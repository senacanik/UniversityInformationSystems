	...
	public partial class ListStudent : Form
	{
		...
		private void Button2_Click(object sender, EventArgs e)
		{
			try
			{
				...
				"WHERE tcNo=@tcNo";  //tcNo parametresine bağlı olarak users bilgilerini çeken sql kodu
				baglanti.Open();
				SqlCommand komut = new SqlCommand(kayit, baglanti);
				komut.Parameters.AddWithValue("@tcNo", textBox1.Text);//tcNo parametremize textbox'dan girilen değeri aktarıyoruz.
				SqlDataReader dr = komut.ExecuteReader(); //SqlDataAdapter da = new SqlDataAdapter(komut);
				if (dr.Read()) //Sadece tek bir kayıt döndürüleceği için while kullanmadım.
				{
					...
					if (dr.Read())
					{
						...
						MessageBox.Show("tebrikler öğrenci girişi başarılı ");
					}
			...
			{
				MessageBox.Show("Hata Uyarısı." + hata.Message);
			}
		}
		private void Button3_Click(object sender, EventArgs e)
		{
			...
			komut.Parameters.AddWithValue("@tcNo", textBox1.Text.Trim());
		...
		private void Button5_Click(object sender, EventArgs e)
		{
			...
			if (dr.Read())
			{
				textBox1.Text = dr["tcNo"].ToString();
				dr.Close();
				DialogResult onay = MessageBox.Show("Öğrenciyi silmek istediğinize emin misiniz ?", "Silme onayı", MessageBoxButtons.YesNo);
				if (DialogResult.Yes == onay)
				{
					...
					MessageBox.Show("Öğrenci Kayıt Silindi..");
					TextClear();
				}
				else
				{
					MessageBox.Show("Öğrenci Kayıdı Bulunamadı..");
					baglanti.Close();
				}
